...
export default function BoxDetailPage() {
    ...
    // --- Déplacement : entrer en mode déplacement ---
        ...
        try {
            await Promise.all(
                selectedPokemons.map(async (pokemonId) => {
                    const res = await fetch(`http://localhost:8000/pokemons/${pokemonId}`, {
                        method: "PATCH",
                        headers: {
                            Authorization: `Bearer ${token}`,
                            "Content-Type": "application/json",
                        },
                        body: JSON.stringify({ boxId: Number(destinationBoxId) }), // UNIQUEMENT boxId
                    });
                    if (!res.ok) throw new Error();
                })
            );

            // Rechargement de la boîte
            setLoading(true);
                ...
                {isMoving && (
                    <section className="mt-6 bg-emerald-950/80 border border-emerald-700 rounded-xl p-4 space-y-3">
                        <h2 className="text-lg font-bold text-amber-200">
                            Déplacer {selectedPokemons.length} Pokémon(s) en les selectionnant
                        </h2>
                ...
                <section aria-label="Pokémons contenus dans la boîte">
                    {box.pokemons.length === 0 ? (
                        ...
                        ${
                                            ...
                                            aria-label={
                                                isMoving
                                                    ? `Sélectionner ${p.name} pour déplacement`
                                                    : `Voir le Pokémon ${p.name}, espèce ${p.species}, niveau ${p.level}${
                                                        p.isShiny ? ", chromatique" : ""
                                                    }`
                                            }
                                        >
